...
## 版本更新记录
- v1.0.1
  - 串口下拉列表显示设备描述 + 端口（设备管理器风格），并支持中间省略显示
  - 新增“当前接收到的数据”版块，按毫秒时间戳追加消息；开始转发时自动清空
  - 接收框限制最大行数以控内存（默认 1000 行）
  - 停止逻辑修复：改用 `siot.stop()` 并清理回调，停止后不再追加数据
  - 标题加入版本号并调整窗口高度以容纳新版块
- v1.0.0
  - 初始版本：订阅指定 Topic 并转发到串口，支持回环串口与基本图形界面
